...
#ubuntu.18.04-x64
$inPath = Join-Path (Get-Item -Path "./").FullName 'Shark.Server/bin/Release/netcoreapp3.1/linux-x64/publish'
$outPath = Join-Path (Get-Item -Path "./").FullName 'shark-server-linux-x64.zip'
[IO.Compression.ZipFile]::CreateFromDirectory($inPath, $outPath, [System.IO.Compression.CompressionLevel]::Optimal, $False)

$inPath = Join-Path (Get-Item -Path "./").FullName 'Shark.Client/bin/Release/netcoreapp3.1/linux-x64/publish'
