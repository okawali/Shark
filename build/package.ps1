...
#win-x64
$inPath = Join-Path (Get-Item -Path "./").FullName 'Shark.Server/bin/Release/net10.0/win-x64/publish'
...
$inPath = Join-Path (Get-Item -Path "./").FullName 'Shark.Client/bin/Release/net10.0/win-x64/publish'
$outPath = Join-Path (Get-Item -Path "./").FullName 'shark-win-x64.zip'
[IO.Compression.ZipFile]::CreateFromDirectory($inPath, $outPath, [System.IO.Compression.CompressionLevel]::Optimal, $False)

#osx-x64
$inPath = Join-Path (Get-Item -Path "./").FullName 'Shark.Server/bin/Release/net10.0/osx-x64/publish'
$outPath = Join-Path (Get-Item -Path "./").FullName 'shark-server-osx-x64.zip'
[IO.Compression.ZipFile]::CreateFromDirectory($inPath, $outPath, [System.IO.Compression.CompressionLevel]::Optimal, $False)

$inPath = Join-Path (Get-Item -Path "./").FullName 'Shark.Client/bin/Release/net10.0/osx-x64/publish'
$outPath = Join-Path (Get-Item -Path "./").FullName 'shark-osx-x64.zip'
[IO.Compression.ZipFile]::CreateFromDirectory($inPath, $outPath, [System.IO.Compression.CompressionLevel]::Optimal, $False)


#ubuntu.18.04-x64
$inPath = Join-Path (Get-Item -Path "./").FullName 'Shark.Server/bin/Release/net10.0/linux-x64/publish'
$outPath = Join-Path (Get-Item -Path "./").FullName 'shark-server-linux-x64.zip'
[IO.Compression.ZipFile]::CreateFromDirectory($inPath, $outPath, [System.IO.Compression.CompressionLevel]::Optimal, $False)

$inPath = Join-Path (Get-Item -Path "./").FullName 'Shark.Client/bin/Release/net10.0/linux-x64/publish'
